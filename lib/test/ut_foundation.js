...
describe('Constructor Check', function() {
    var foundPayload = new FoundPayload('read');

    it('FoundPayload()', function () {
        should(foundPayload.cmd).be.equal('read');
        should(foundPayload.params).be.deepEqual(foundMeta.getParams('read'));
    });
});

describe('Signature Check', function () {
    it('frame()', function () {
        var readPayload = new FoundPayload('read'),
            discoverPayload = new FoundPayload('discover');

        (function() { readPayload.frame([]); }).should.not.throw();
        (function() { readPayload.frame({}); }).should.throw();
        (function() { readPayload.frame('xxx'); }).should.throw();
        (function() { readPayload.frame(123); }).should.throw();
        (function() { readPayload.frame(true); }).should.throw();
        (function() { readPayload.frame(undefined); }).should.throw();
        (function() { readPayload.frame(null); }).should.throw();

        (function() { discoverPayload.frame([]); }).should.throw();
        (function() { discoverPayload.frame('xxx'); }).should.throw();
        (function() { discoverPayload.frame(123); }).should.throw();
        (function() { discoverPayload.frame(undefined); }).should.throw();
        (function() { discoverPayload.frame(null); }).should.throw();
    });

    it('parse()', function () {
        var readPayload = new FoundPayload('read');

        (function() { readPayload.parse(new Buffer([])); }).should.not.throw();
        (function() { readPayload.parse({}); }).should.throw();
        (function() { readPayload.parse([]); }).should.throw();
        (function() { readPayload.parse('xxx'); }).should.throw();
        (function() { readPayload.parse(123); }).should.throw();
        (function() { readPayload.parse(true); }).should.throw();
        (function() { readPayload.parse(undefined); }).should.throw();
        (function() { readPayload.parse(null); }).should.throw();
    });
});
...
var valObj = {
        ...
        configReportRsp: [
            {status: 0, direction: 0, attrId: 0x0002},
...
describe('framer() and parser() functional check', function () {
    var cmds = ['read', 'readRsp', 'write', 'writeUndiv', 'writeRsp', 'writeNoRsp', 'configReport' , 
                'configReportRsp','readReportConfig', 'readReportConfigRsp', 'report', 'defaultRsp',
                'discover', 'discoverRsp', 'readStruct',  'writeStrcut', 'writeStrcutRsp'];

    _.forEach(cmds, function (cmd) {
        it(cmd + '()', function (done) {
            var cmdPayload = new FoundPayload(cmd),
                zBuf = cmdPayload.frame(valObj[cmd]);

            cmdPayload.parse(zBuf, function (err, result) {
                if (_.isEqual(valObj[cmd], result)) done();
            });
        });
    });
});