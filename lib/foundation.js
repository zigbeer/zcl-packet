...
var FoundPayload = function(cmd) {
    this.cmd = cmd;
    this.params = foundMeta.getParams(cmd);
};
...
FoundPayload.prototype.frame = function (payload) {
    ...
    switch (this.cmd) {
        ...
        case 'discoverRsp':
            ...
            _.forEach(payload.attrInfos, function (attrInfo) {
        ...
        default:
            ...
            _.forEach(payload, function (argObj) {
...
FoundPayload.prototype.parse = function (bBuf, callback) {
    var self = this,
        getObjCB,
        parsedData;
    ...
    switch (this.cmd) {
        case 'defaultRsp':
        case 'discover':
            this._getObj(bBuf, function (err, result) {
        ...
        case 'discoverRsp':
            parsedData = {
                discComplete: bBuf.readUInt8(0),
                attrInfos: []
            };

            bBuf = bBuf.slice(1);
            parsedBufLen += 1;
            this._getObj(bBuf, getObjCB);
            break;
...
FoundPayload.prototype._getObj = function (buf, callback) {
    var chunkRules = [],
        parser,
        knownBufLen = foundMeta.get(this.cmd).knownBufLen,
        result = {
            data: null,
            leftBuf: null
        };
...
function getChunkBuf (rule, arg) {
    ...
    switch (rule) {
        ...
        case 'selector':
            ...
            for (var i = 0; i < arg.indicator; i += 1) {
    ...
    this.tap(function () {
        if (this.vars.direction === 0) {
            ...
            this.uint8('dataType').uint16('minRepIntval').uint16('maxRepIntval').tap(function () {
                analogOrDigital = isDataAnalogDigital(this.vars.dataType);
