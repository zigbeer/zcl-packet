...
var zclMeta = JSON.parse(fs.readFileSync(__dirname + '/defs/zcl_meta.json')),
    zclDefs = JSON.parse(fs.readFileSync(__dirname + '/defs/zcl_defs.json'));

var zclmeta = {
    foundation: {},
    functional: {},
    FoundationId: new Enum(zclDefs.FoundationId),
    ClusterId: new Enum(zclDefs.ClusterId),
    Direction: new Enum(zclDefs.Direction),
    DataType: new Enum(zclDefs.DataType),
    StatusCode: new Enum(zclDefs.StatusCode)
};

zclDefs.FoundationId = null;
zclDefs.ClusterId = null;
zclDefs.Direction = null;
zclDefs.DataType = null;
zclDefs.StatusCode = null;
zclDefs = null;
...
zclmeta.foundation.getCmdId = function (cmd) {
    var meta = this.get(cmd);

    return meta ? meta.cmdId : undefined;        // return: "Client To Server", "Server To Client"
};
...
zclmeta.functional.getCommand = function (cluster, dir, cmd) {
    var meta = zclMeta.functional[cluster],
        command = dir ? meta.cmdRsp : meta.cmd,    // [ [ cmd, cmdId ], .... ]
        output = {};

    command.forEach(function (item, idx) {
        if (item[0] === cmd || item[1] === cmd) {
            output.cmd = item[0];
            output.cmdId = item[1];
        }
    });
